...
#            ※ eval λ 라벨이 WS 와 반대로 보일 수 있으나 표기일 뿐(swapped(λ)=unswapped(1-λ),
#              λ 균등샘플 → 학습 동치). reward 스왑 금지 — 거슬리면 plot 에서만 1-λ 표기.
...
#            ⚠ α↑ 는 front 를 가운데로 과수축(collapse)시켜 학습·coverage 악화. α=1 이 known-good
#            (0527_pbi); α=2/3 은 학습 망가짐(eval q@λ1·nd_count 급락) — 키우지 말 것.
SCALARIZATION = 'ws'      # 'ws' | 'tch' | 'pbi'
TCH_RHO       = 0.0       # augmented Tchebycheff 증강항 계수 ρ (0 = 순수 TCH)
PBI_ALPHA     = 1.0       # PBI 수직거리 d2 벌점 계수 α — α=1 known-good. 키우면 collapse(위 주석).
...
def sil_pomo_rollout(env: HFSPGraphEnv, model: FFSPModel,
    ...
    if SCALARIZATION == 'ws':
        ...
        # d1 = (e·λ_vec)/‖λ_vec‖ (λ 방향 투영), d2 = ‖e - d1·λ̂‖ (λ-ray 수직 이탈), λ̂=λ_vec/‖λ_vec‖.
        # min g = d1 + α·d2  →  보상은 -g. d2 항이 극단 λ 에서도 반대 목적 기울기를 살림.
